...
local Window = WindUI:CreateWindow({
    Title = "<font color='#FFB6C1'>X</font><font color='#FFA0B5'>I</font><font color='#FF8AA9'>A</font><font color='#FF749D'>O</font><font color='#FF5E91'>X</font><font color='#FF4885'>I</font> <font color='#FF3279'>V</font><font color='#FF1C6D'>4</font>",
    Author = "by 小西/凌乱",
    Folder = "HackerHub",
    Size = UDim2.fromOffset(200, 395),
    KeySystem = {
        Key = { "凌乱NB", "小西nb", "宇星辰", "阵雨眉目" }, 
        Note = "请输入卡密",
        SaveKey = false,
    },
    Transparent = true,
...
do
    ...
    Tabs.MusicTab = Tabs.MusicSection:Tab({ Title = "背景音乐", Icon = "music" })
end
...
Tabs.MoneyTab:Toggle({
    ...
    Default = false,
    Callback = function(Value)
        autoRobBankCombo = Value
        
        if autoRobBankCombo then
            coroutine.wrap(function()
                warn("[自动抢银行] 已启动")
                while autoRobBankCombo and task.wait(1) do
                    local success, err = pcall(function()
                        ...
                        -- 1. 定位银行与金库
                        local bank = workspace.Game.Buildings:FindFirstChild("Bank")
                        local vault = bank and bank:FindFirstChild("Vault")
                        local cashBundleFolder = workspace.Game.Entities:FindFirstChild("CashBundle")

                        if not bank or not vault then
                            warn("[自动抢银行] 未找到银行/金库，5秒后重试")
                            task.wait(5)
                            return
                        end

                        -- 2. 传送到银行
                        HRP.CFrame = CFrame.new(bank.Position + Vector3.new(0, 3, 0))
                        task.wait(2)

                        -- 3. 触发破解金库
                        game:GetService("ReplicatedStorage").RemoteEvents.crackVault:FireServer(vault)
                        warn("[自动抢银行] 正在破解金库，等待8秒...")
                        task.wait(8)

                        -- 4. 自动拾取所有现金
                        if cashBundleFolder then
                            for _, cash in pairs(cashBundleFolder:GetChildren()) do
                                if cash and cash:IsA("BasePart") then
                                    HRP.CFrame = cash.CFrame
                                    task.wait(0.3)
                                    local prompt = cash:FindFirstChildOfClass("ProximityPrompt")
                                    if prompt then
                                        fireproximityprompt(prompt, 0.5)
                                    end
                                end
                            end
                            warn("[自动抢银行] 本轮拾取完成，等待60秒刷新...")
                        end

                        -- 5. 等待刷新循环
                        local waitTime = 60
                        while waitTime > 0 and autoRobBankCombo do
                            task.wait(1)
                            waitTime = waitTime - 1
                        end
                    end)
